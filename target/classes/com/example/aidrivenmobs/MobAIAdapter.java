...
public class MobAIAdapter {
    ...
    public void applyAdaptiveBehaviors(Entity entity) {
        if (entity instanceof Mob) {
            ...
            if (entity instanceof Creeper) {
                ...
                applyPiglinBehaviors((Piglin) entity);
            } else if (entity instanceof Hoglin) {
                applyHoglinBehaviors((Hoglin) entity);
            } else if (entity instanceof Ghast) {
                applyGhastBehaviors((Ghast) entity);
            } else if (entity instanceof Blaze) {
                applyBlazeBehaviors((Blaze) entity);
            } else if (entity instanceof WitherSkeleton) {
                applyWitherSkeletonBehaviors((WitherSkeleton) entity);
                ...
                applyWardenBehaviors((Warden) entity);
            }
        }
    }
    ...
    /**
     * Apply piglin-specific behaviors
     */
    private void applyPiglinBehaviors(Piglin piglin) {
        // Remember trading behavior
        if (piglin.getTarget() != null) {
            PiglinTradeMemoryGoal goal = new PiglinTradeMemoryGoal(plugin, piglin);
            if (goal.canStart()) {
                goal.start();
                activeGoals.put(piglin.getUniqueId(), goal);
                plugin.getLogger().info("Applied PiglinTradeMemoryGoal to piglin at " + piglin.getLocation());
            }
        }
    }

    /**
     * Apply hoglin-specific behaviors
     */
    private void applyHoglinBehaviors(Hoglin hoglin) {
        // Avoid traps
        if (hoglin.getTarget() != null) {
            HoglinTrapAvoidanceGoal goal = new HoglinTrapAvoidanceGoal(plugin, hoglin);
            if (goal.canStart()) {
                goal.start();
                activeGoals.put(hoglin.getUniqueId(), goal);
                plugin.getLogger().info("Applied HoglinTrapAvoidanceGoal to hoglin at " + hoglin.getLocation());
            }
        }
    }
    ...
    /**
     * Apply wither skeleton-specific behaviors
     */
    private void applyWitherSkeletonBehaviors(WitherSkeleton witherSkeleton) {
        // Squad formations
        if (witherSkeleton.getTarget() != null) {
            WitherSkeletonFormationGoal goal = new WitherSkeletonFormationGoal(plugin, witherSkeleton);
            if (goal.canStart()) {
                goal.start();
                activeGoals.put(witherSkeleton.getUniqueId(), goal);
                plugin.getLogger().info(
                        "Applied WitherSkeletonFormationGoal to wither skeleton at " + witherSkeleton.getLocation());
            }
        }
    }
    ...
    /**
     * Apply warden-specific behaviors
     */
    private void applyWardenBehaviors(Warden warden) {
        // Track sound patterns
        if (warden.getTarget() != null) {
            WardenSoundTrackingGoal goal = new WardenSoundTrackingGoal(plugin, warden);
            if (goal.canStart()) {
                goal.start();
                activeGoals.put(warden.getUniqueId(), goal);
                plugin.getLogger().info("Applied WardenSoundTrackingGoal to warden");
            }
        }
    }
